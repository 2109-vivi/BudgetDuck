...
const StackedBarCategoryLegend = (data) => {
  return (
    <div className='category-bargraph-legend-container' style={{ flexWrap: 'wrap' }}>
      {data.payload.map((item, index) => {
        return (
          <div key={index}>
            <span fill={item.color} className='category-bargraph-legend-icon' style={{ background: item.color }} />
